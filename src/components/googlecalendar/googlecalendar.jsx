...
import { googleCalendarMCardService } from '../../services/google-calendar-mcard-service.ts';
...
const GoogleCalendar = ({ className = '' }) => {
  ...
  const sendEventsToMCard = async (events) => {
    try {
      ...
      await googleCalendarMCardService.saveReadableSummaryToMCard(events, userEmail);

      setMcardSyncStatus('success');
      console.log(`✅ Successfully synced ${result.savedCount} events to MCard`);
      
      // Show success message briefly
      setTimeout(() => setMcardSyncStatus(null), 3000);
      
    } catch (error) {
      console.error('❌ Error syncing events to MCard:', error);
